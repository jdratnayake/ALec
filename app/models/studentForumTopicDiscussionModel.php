...
class StudentForumTopicDiscussionModel extends Database
{
    ...
    {
        $query = "SELECT topic_id, subject, question, DATE_FORMAT(post_time, '%d %M %Y') AS post_time, forum_topic.user_id, random_status, CONCAT(first_name, ' ', last_name) AS name, CONCAT(random_first_name, ' ', random_last_name) AS random_name, user_type FROM forum_topic INNER JOIN user ON forum_topic.user_id=user.user_id LEFT JOIN student ON student.user_id=user.user_id WHERE topic_id='$topicId' LIMIT 1";
